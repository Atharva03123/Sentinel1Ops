...
def get_cpu_temperature():
    """
    Try to read real CPU temp via psutil.
    Falls back to a realistic simulation if sensors unavailable.
    ...
    # Simulation: base temp tied to CPU usage for realism
    cpu_pct = psutil.cpu_percent(interval=0.1)
    base_temp = 45.0
    load_contrib = cpu_pct * 0.42          # high load → higher temp
...
def get_ai_response(question, metrics_context):
    """Rule-based AI assistant for system diagnostics"""
    q = question.lower()
    ...
    if any(w in q for w in ["cpu", "processor", "compute"]):
        if cpu > 90:
            responses.append(f"🔥 **CPU CRITICAL at {cpu}%!**\n\n**Severity:** CRITICAL\n\n**Root Cause:** System is overloaded. Processes are competing for CPU cycles.\n\n**Solutions:**\n- Run `ps aux --sort=-%cpu | head -15` to find culprits\n- Kill non-essential processes\n- Consider horizontal scaling or load balancing\n- Check for infinite loops or runaway threads")
        elif cpu > 80:
            responses.append(f"⚠️ **CPU HIGH at {cpu}%**\n\n**Severity:** WARNING\n\n**Root Cause:** Elevated load — approaching saturation threshold.\n\n**Solutions:**\n- Monitor with `htop` for spike patterns\n- Check scheduled jobs (`crontab -l`)\n- Review recent deployments that may have introduced load")
        else:
            responses.append(f"✅ **CPU Normal at {cpu}%**\n\n**Severity:** NORMAL\n\nCPU is within safe operating range (below 80%). No action needed. Continue monitoring.")

    if any(w in q for w in ["temp", "temperature", "thermal", "heat", "hot"]):
        temp_thresh = admin_config.get("alert_thresholds", {}).get("cpu_temp", 80)
        if temp > temp_thresh:
            responses.append(f"🌡️ **CPU TEMPERATURE CRITICAL at {temp}°C!**\n\n**Severity:** CRITICAL\n\n**Root Cause:** CPU is overheating — thermal throttling may activate reducing performance.\n\n**Solutions:**\n- Check CPU cooler seating and thermal paste\n- Clean dust from heatsink/fans\n- Improve case airflow\n- Reduce background CPU load\n- Check ambient room temperature\n- Consider undervolting if safe")
        elif temp > temp_thresh - 10:
            responses.append(f"⚠️ **CPU Temperature WARNING at {temp}°C**\n\n**Severity:** WARNING\n\n**Root Cause:** Temperature approaching critical threshold ({temp_thresh}°C).\n\n**Solutions:**\n- Monitor with `sensors` command\n- Ensure fans are spinning at correct RPM\n- Consider reapplying thermal compound")
        else:
            responses.append(f"✅ **CPU Temperature Normal at {temp}°C**\n\n**Severity:** NORMAL\n\nCPU is running at safe temperature. Threshold set at {temp_thresh}°C.")

    if any(w in q for w in ["memory", "ram", "mem", "heap"]):
        if mem > 90:
            responses.append(f"🔥 **MEMORY CRITICAL at {mem}%!**\n\n**Severity:** CRITICAL\n\n**Root Cause:** System is at risk of OOM (Out of Memory) kill events.\n\n**Solutions:**\n- Check swap: `free -h`\n- Find memory hogs: `ps aux --sort=-%mem | head -10`\n- Restart memory-leaking services\n- Consider adding swap space temporarily")
        elif mem > 85:
            responses.append(f"⚠️ **Memory HIGH at {mem}%**\n\n**Severity:** WARNING\n\n**Root Cause:** Memory pressure detected — possible memory leak.\n\n**Solutions:**\n- Investigate with `valgrind` or heap profiler\n- Restart services with elevated memory consumption\n- Schedule maintenance window")
        else:
            responses.append(f"✅ **Memory Normal at {mem}%**\n\n**Severity:** NORMAL\n\nMemory is healthy. No leaks detected. Continue monitoring above 85%.")

    if any(w in q for w in ["disk", "storage", "space", "drive"]):
        if disk > 95:
            responses.append(f"🚨 **DISK CRITICAL at {disk}%!**\n\n**Severity:** CRITICAL\n\n**Root Cause:** Disk is nearly full — I/O operations may start failing.\n\n**Solutions:**\n- Find large files: `du -sh /* | sort -rh | head -20`\n- Clear old logs: `journalctl --vacuum-time=2d`\n- Delete temp files: `rm -rf /tmp/*`\n- Archive or move data to external storage")
        elif disk > 90:
            responses.append(f"⚠️ **Disk HIGH at {disk}%**\n\n**Severity:** WARNING\n\n**Root Cause:** Disk usage elevated — plan cleanup.\n\n**Solutions:**\n- Schedule log rotation\n- Review backup retention policies\n- Consider expanding disk capacity")
        else:
            responses.append(f"✅ **Disk Normal at {disk}%**\n\n**Severity:** NORMAL\n\nDisk is safe. Start planning cleanup above 85%.")

    if any(w in q for w in ["health", "status", "overall", "summary"]):
        if health >= 70:
            responses.append(f"💚 **System Health: GOOD ({health}/100)**\n\n**Severity:** NORMAL\n\nAll metrics within acceptable range. System is performing well.\n\n- CPU: {cpu}% ✅\n- Memory: {mem}% ✅\n- Disk: {disk}% ✅\n- CPU Temp: {temp}°C ✅\n- Health Score: {health}/100")
        elif health >= 50:
            responses.append(f"🟡 **System Health: WARNING ({health}/100)**\n\n**Severity:** WARNING\n\nSome metrics need attention. Review the dashboard.\n\n- CPU: {cpu}%\n- Memory: {mem}%\n- Disk: {disk}%\n- CPU Temp: {temp}°C")
        else:
            responses.append(f"🔴 **System Health: CRITICAL ({health}/100)!**\n\n**Severity:** CRITICAL\n\nImmediate action required.\n\n- CPU: {cpu}% {'🔴' if cpu > 80 else '✅'}\n- Memory: {mem}% {'🔴' if mem > 85 else '✅'}\n- Disk: {disk}% {'🔴' if disk > 90 else '✅'}\n- CPU Temp: {temp}°C {'🔴' if temp > 80 else '✅'}")

    if any(w in q for w in ["alert", "threshold", "alarm"]):
        cfg = admin_config.get("alert_thresholds", {})
        responses.append(f"📊 **Current Alert Thresholds:**\n\n- 🔲 CPU: **{cfg.get('cpu',80)}%**\n- 🧠 Memory: **{cfg.get('memory',85)}%**\n- 💾 Disk: **{cfg.get('disk',90)}%**\n- 🌡️ CPU Temp: **{cfg.get('cpu_temp',80)}°C**\n\nThresholds can be modified in the Admin Panel → Thresholds tab.")

    if any(w in q for w in ["log", "logs", "history"]):
        log_count = len(st.session_state.get("log_entries", []))
        responses.append(f"📄 **Logs Overview:**\n\nCurrently tracking **{log_count}** log entries in memory.\n\nLogs are also stored in `sentinelops.log` file.\n\n- Use `tail -f sentinelops.log` in terminal for live streaming\n- Admin can filter, download as TXT/CSV, and clear logs\n- Logs include Auth, Metrics, Alerts, and System events")
    ...
    if any(w in q for w in ["process", "top", "pid"]):
        try:
            ...
            responses.append("⚠️ Unable to retrieve process list.")

    if any(w in q for w in ["help", "what can you", "commands"]):
        responses.append("""🤖 **SentinelOps AI — Commands & Capabilities:**

**System Analysis:**
- *"Analyze CPU"* — CPU diagnosis with severity
- *"Analyze Memory"* — Memory health analysis
- *"System Health"* — Full health summary
- *"CPU Temperature"* — Thermal analysis

**Diagnostics:**
- *"Why is CPU high?"* — Root cause analysis
- *"Top processes"* — Process CPU/MEM breakdown
- *"Is disk safe?"* — Disk usage check

**Configuration:**
- *"Show alert thresholds"* — Current limits
- *"Show logs"* — Log system overview

**Quick Tips:**
- Ask about **cpu**, **memory**, **disk**, **temperature**, **health**, **alerts**, **logs**, **processes**
- Ask **"why are there issues?"** for full RCA""")
    ...
    if not responses:
        temp_thresh = admin_config.get("alert_thresholds", {}).get("cpu_temp", 80)
        responses.append(f"""🤖 **SentinelOps AI Assistant** — Live Snapshot:
...
Ask me about: **cpu**, **memory**, **disk**, **temperature**, **health**, **alerts**, **logs**, **processes**, or type **"help"** for all commands.""")
...
def generate_pdf_report(cpu, memory, disk, health_score, health_color_name, alerts_list, log_entries, cpu_temp=0):
    ...
    section_style = ParagraphStyle(
        ...
        fontSize=13, textColor=colors.HexColor('#0a3d62'),
    ...
    story.append(Paragraph("🛡️ SentinelOps", title_style))
    ...
    summary_data = [
        ...
        ["CPU Usage", f"{cpu}%",
         "NORMAL" if cpu < 80 else ("WARNING" if cpu < 90 else "CRITICAL"),
         f"{admin_config['alert_thresholds']['cpu']}%"],
        ["Memory Usage", f"{memory}%",
         "NORMAL" if memory < 85 else ("WARNING" if memory < 90 else "CRITICAL"),
         f"{admin_config['alert_thresholds']['memory']}%"],
        ["Disk Usage", f"{disk}%",
         "NORMAL" if disk < 90 else ("WARNING" if disk < 95 else "CRITICAL"),
         f"{admin_config['alert_thresholds']['disk']}%"],
        ["CPU Temperature", f"{cpu_temp}°C",
         "NORMAL" if cpu_temp < temp_thresh else "CRITICAL",
         f"{temp_thresh}°C"],
        ["Health Score", f"{health_val}/100", health_label, "≥70"],
    ]
    ...
    story.append(Paragraph("System Information", section_style))
    sys_data = [
        ["Property", "Value"],
        ...
        ["Report User", st.session_state.get("current_user", "Unknown")],
        ["Report Time", now.strftime('%Y-%m-%d %H:%M:%S')],
    ]
    sys_table = Table(sys_data, colWidths=[5*cm, 9*cm])
    ...
    if alert_history:
        alert_data = [["Time", "Severity", "Component", "Message"]]
        ...
        alert_table = Table(alert_data, colWidths=[3.5*cm, 2.5*cm, 2.5*cm, 5.5*cm])
        alert_ts = TableStyle([
        ...
        for i, row in enumerate(alert_data[1:], 1):
            if row[1] == "CRITICAL":
                alert_ts.add('TEXTCOLOR', (1, i), (1, i), colors.HexColor('#c0392b'))
                alert_ts.add('FONTNAME', (1, i), (1, i), 'Helvetica-Bold')
            elif row[1] == "WARNING":
                alert_ts.add('TEXTCOLOR', (1, i), (1, i), colors.HexColor('#e67e22'))
        alert_table.setStyle(alert_ts)
        story.append(alert_table)
    else:
        story.append(Paragraph("✅ No active alerts recorded.", normal_style))
    ...
    story.append(Paragraph(
        f"SentinelOps Enterprise Health Engine v2.0  ·  Generated by {st.session_state.get('current_user', 'Unknown')}  ·  {now.strftime('%Y-%m-%d %H:%M:%S')}  ·  CONFIDENTIAL",
...
# Admin panel re-auth (admin must re-login to access admin panel from user dashboard)
if "admin_panel_auth" not in st.session_state:
    st.session_state.admin_panel_auth = False

# ══════════════════════════════════════════════════════════
#  LOGIN PAGE — ROLE SELECTION + TWO SEPARATE LOGIN FLOWS
# ══════════════════════════════════════════════════════════
if not st.session_state.logged_in:
    ...
    .app-sub2{font-size:11px;color:#164e63;font-family:'JetBrains Mono',monospace;letter-spacing:0.12em;text-align:center;overflow:hidden;white-space:nowrap;border-right:2px solid #06b6d4;width:0;margin:0 auto 1.5rem;animation:typeIt 2.5s steps(28) 1s forwards,cursorBlink 0.8s step-end infinite;}
    @keyframes typeIt{to{width:240px;}}
    ...
    .role-card{position:relative;z-index:10;width:100%;max-width:480px;margin:0 auto;background:rgba(2,15,35,0.85);border:1px solid rgba(6,182,212,0.2);border-radius:24px;padding:2.4rem 2rem;backdrop-filter:blur(20px);animation:cardIn 0.8s cubic-bezier(0.16,1,0.3,1) both;}
    .admin-login-card{position:relative;z-index:10;width:100%;max-width:420px;margin:0 auto;background:rgba(2,15,35,0.90);border:1px solid rgba(250,204,21,0.25);border-radius:24px;padding:2.8rem 2.4rem;backdrop-filter:blur(20px);animation:cardIn 0.8s cubic-bezier(0.16,1,0.3,1) both,adminPulse 4s ease-in-out 1s infinite alternate;}
    ...
    if st.session_state.login_mode is None:
        ...
        <div class="role-card">
            ...
            <div class="app-sub2">ENTERPRISE HEALTH ENGINE v2.0</div>
    ...
    # ── Step 2a: USER LOGIN ──
    elif st.session_state.login_mode == "user":
        st.markdown("""
        <div class="login-card">
            <div class="logo-ring">👤</div>
            <div class="app-title">User Access</div>
            <div class="app-sub2">SENTINELOPS DASHBOARD</div>
            <div class="welcome-txt">
                <h3>User Sign In</h3>
                <p>Access dashboard, metrics, graphs & reports</p>
            </div>
        </div>
        """, unsafe_allow_html=True)
            ...
            if st.button("Login →", use_container_width=True, key="user_login_btn"):
                if not username:
                    ...
                    if username in users and users[username]["password_hash"] == ph:
                        ...
                        if role == "admin":
                            st.error("❌ Admin accounts must use the Admin Login portal.")
                            record_login(username, role, False)
                            log_event("WARNING", "Auth", f"Admin '{username}' attempted user login portal")
                        else:
                            st.session_state.logged_in = True
                            st.session_state.current_user = username
                            st.session_state.user_role = role
                            record_login(username, role, True)
                            log_event("INFO", "Auth", f"User '{username}' logged in via User Portal")
                            st.rerun()
                    else:
                        record_login(username, "unknown", False)
                        log_event("WARNING", "Auth", f"Failed user login attempt for '{username}'")
                        st.error("❌ Invalid username or password")
            if st.button("← Back", use_container_width=True, key="user_back"):
                st.session_state.login_mode = None
                st.rerun()
            st.markdown("""
            <div class="creds-hint">viewer / view123</div>
            <div class="status-bar2">
                <div class="status-dot2"></div>
                <span>USER PORTAL · SECURE</span>
            </div>""", unsafe_allow_html=True)
    ...
    # ── Step 2b: ADMIN LOGIN ──
        ...
        <div class="admin-login-card">
            ...
            <div class="app-sub2" style="border-right-color:#fbbf24;">RESTRICTED ACCESS · ADMIN ONLY</div>
            <div class="welcome-txt">
                ...
                if not admin_uname:
                    ...
                    if admin_uname in users and users[admin_uname]["password_hash"] == ph:
                        ...
                        if role != "admin":
                            ...
                            log_event("WARNING", "Auth", f"Non-admin '{admin_uname}' attempted Admin Portal")
                        else:
                            st.session_state.logged_in = True
                            st.session_state.current_user = admin_uname
                            st.session_state.user_role = role
                            st.session_state.admin_panel_auth = True  # Admin logged in directly
                            record_login(admin_uname, role, True)
                            log_event("INFO", "Auth", f"Admin '{admin_uname}' logged in via Admin Portal")
                            st.rerun()
                    else:
                        record_login(admin_uname, "unknown", False)
                        log_event("WARNING", "Auth", f"Failed admin login attempt for '{admin_uname}'")
                        st.error("❌ Invalid administrator credentials")
            if st.button("← Back", use_container_width=True, key="admin_back"):
                st.session_state.login_mode = None
                st.rerun()
            st.markdown("""
            <div class="creds-hint" style="border-color:rgba(250,204,21,0.2);color:#92400e;">
                admin / sentinel123
            </div>
            <div class="status-bar2">
                <div class="status-dot2" style="background:#fbbf24;"></div>
                <span style="color:#78350f;">ADMIN PORTAL · HIGH SECURITY</span>
            </div>""", unsafe_allow_html=True)
...
<style>
    ...
    .stTextInput input, .stTextArea textarea, .stSelectbox select { background: #161b22 !important; border: 1px solid #30363d !important; color: #e6edf3 !important; border-radius: 8px !important; font-family: 'JetBrains Mono', monospace !important; }
    ...
    .chat-bubble-user { background: #1c2a3d; border: 1px solid #1f4068; border-radius: 12px 12px 4px 12px; padding: 10px 14px; margin: 6px 0; font-size: 0.85rem; color: #79c0ff; font-family: 'Inter', sans-serif; }
    .chat-bubble-ai { background: #161b22; border: 1px solid #21262d; border-left: 3px solid #3fb950; border-radius: 12px 12px 12px 4px; padding: 10px 14px; margin: 6px 0; font-size: 0.85rem; color: #e6edf3; font-family: 'Inter', sans-serif; }
    @keyframes alertPulse{0%,100%{opacity:1;}50%{opacity:0.7;}}
</style>
""", unsafe_allow_html=True)

# ─────────────────────────────
# COLLECT LIVE METRICS + TEMP
# ─────────────────────────────
...
# ─────────────────────────────
# AUTO ALERT CHECK (with cooldown)
# ─────────────────────────────
...
for metric, key, threshold in [
    ("CPU", "cpu_percent", thresholds["cpu"]),
    ("Memory", "memory_percent", thresholds["memory"]),
    ("Disk", "disk_percent", thresholds["disk"]),
]:
...
# Check CPU Temperature (separate cooldown key)
...
with col_status:
    db_badge = (
        '<span style="display:inline-block; padding:2px 10px; border-radius:20px; font-size:0.7rem; font-family:\'JetBrains Mono\',monospace; font-weight:600; letter-spacing:0.05em; background:#1a2d1e; color:#3fb950; border:1px solid #3fb950;">● LIVE DB</span>'
        if DB_AVAILABLE else
        '<span style="display:inline-block; padding:2px 10px; border-radius:20px; font-size:0.7rem; font-family:\'JetBrains Mono\',monospace; font-weight:600; letter-spacing:0.05em; background:#2d1a1a; color:#e3b341; border:1px solid #e3b341;">⚡ SIMULATED</span>'
    )
...
if alerts_triggered:
    for sev, comp, msg in alerts_triggered:
        ...
        st.markdown(f"""
        <div style="background:rgba(248,81,73,0.08); border:1px solid {color}; border-left:4px solid {color}; border-radius:10px; padding:0.7rem 1.2rem; margin-bottom:8px; font-family:'JetBrains Mono',monospace; font-size:0.82rem; color:{color}; animation:alertPulse 2s ease-in-out infinite;">
            {icon} <strong>[{sev}]</strong> {comp}: {msg}
        </div>
        """, unsafe_allow_html=True)

    if admin_config.get("voice_alerts") and alerts_triggered:
        # CPU Temperature voice alert — special message
        temp_alert = next((a for a in alerts_triggered if "Temperature" in a[1]), None)
        if temp_alert:
            inject_voice_alert("Warning! Your CPU temperature is high. Please check your cooling system.", temp_alert[0])
        else:
            top = alerts_triggered[0]
            inject_voice_alert(f"Alert: {top[1]}. {top[2]}", top[0])

# ─────────────────────────────
# CPU TEMP WARNING BANNER (standalone, outside alert loop)
# ─────────────────────────────
if cpu_temp > temp_threshold:
    st.markdown(f"""
    <div style="background:rgba(248,81,73,0.12); border:1px solid #f85149; border-left:4px solid #f85149; border-radius:10px; padding:0.7rem 1.2rem; margin-bottom:8px; font-family:'JetBrains Mono',monospace; font-size:0.82rem; color:#f85149; animation:alertPulse 1.5s ease-in-out infinite;">
        🌡️ <strong>[THERMAL ALERT]</strong> CPU Temperature: <strong>{cpu_temp}°C</strong> — Exceeds threshold of {temp_threshold}°C! Risk of thermal throttling.
    </div>
    """, unsafe_allow_html=True)
...
if is_admin:
    ...
    tab_logs_ref = tab_logs
    tab_multisys_ref = tab_multisys
    tab_admin_ref = tab_admin
else:
...
with tab_dashboard:
    ...
    # Top Processes
    st.markdown("### 🔍 Top Processes (Root Cause)")
    try:
        proc_list = []
        ...
        st.info("Process list unavailable in this environment.")
    ...
    critical = int((alert_df["severity"] == "CRITICAL").sum()) if not alert_df.empty and "severity" in alert_df.columns else len([a for a in st.session_state.get("alert_history", []) if a.get("severity") == "CRITICAL"])
    warning  = int((alert_df["severity"] == "WARNING").sum())  if not alert_df.empty and "severity" in alert_df.columns else len([a for a in st.session_state.get("alert_history", []) if a.get("severity") == "WARNING"])
    ...
    def status_card(col, icon, label, value, color, sub=""):
        col.markdown(f"""
        <div style="background:#161b22; border:1px solid #21262d; border-radius:14px; padding:1.2rem 1.4rem; text-align:center; border-top:3px solid {color};">
            <div style="font-size:1.8rem; margin-bottom:4px;">{icon}</div>
            <div style="font-family:'JetBrains Mono',monospace; font-size:0.68rem; color:#8b949e; text-transform:uppercase; letter-spacing:0.1em;">{label}</div>
            <div style="font-family:'JetBrains Mono',monospace; font-size:1.8rem; font-weight:700; color:{color}; margin:4px 0;">{value}</div>
    ...
    with fcol1:
        if st.button("🔄 Refresh"):
            st.rerun()
    with fcol2:
        st.markdown('<div style="padding-top:10px; font-family:\'JetBrains Mono\',monospace; font-size:0.72rem; color:#30363d;">SentinelOps v2.0 · Python · PostgreSQL · Streamlit</div>', unsafe_allow_html=True)
...
with tab_rca:
    ...
    st.markdown('<p style="color:#8b949e; font-size:0.85rem; font-family:\'JetBrains Mono\',monospace;">Automated anomaly detection and fix recommendations based on live metrics.</p>', unsafe_allow_html=True)
    ...
    with rca_col2:
        ...
        snap_items = [
            ("CPU", data['cpu_percent'], cpu_color),
            ("Memory", data['memory_percent'], mem_color),
            ("Disk", data['disk_percent'], disk_color),
            ("CPU Temp", f"{cpu_temp}°C", temp_color),
            ("Health", health_score, health_color),
        ]
        for label, val, color in snap_items:
            suffix = "%" if label not in ["CPU Temp", "Health"] else ""
            st.markdown(f"""
            <div style="background:#161b22; border:1px solid #21262d; border-radius:8px; padding:8px 14px; margin-bottom:6px; display:flex; justify-content:space-between; align-items:center;">
                <span style="font-family:'JetBrains Mono',monospace; font-size:0.8rem; color:#8b949e;">{label}</span>
                <span style="font-family:'JetBrains Mono',monospace; font-size:1rem; font-weight:700; color:{color};">{val}{suffix}</span>
            </div>
            """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="background:rgba(22,27,34,0.8); border:1px solid {sev_color}; border-radius:14px; padding:1.4rem 1.8rem; margin-bottom:1rem;">
        <div style="font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:#8b949e; text-transform:uppercase; letter-spacing:0.12em; margin-bottom:8px;">OVERALL SEVERITY</div>
        <div style="font-family:'JetBrains Mono',monospace; font-size:2rem; font-weight:700; color:{sev_color};">{sev}</div>
    </div>
    """, unsafe_allow_html=True)

    # Temperature cause display
    if cpu_temp > temp_threshold:
        st.markdown(f"""
        <div style="background:#161b22; border:1px solid #21262d; border-left:4px solid #f85149; border-radius:8px; padding:10px 14px; margin-bottom:6px; font-family:'JetBrains Mono',monospace; font-size:0.82rem; color:#e6edf3;">
            <strong>🌡️ CPU Thermal Issue</strong> &nbsp;·&nbsp; <span style="color:#8b949e;">CPU at {cpu_temp}°C — exceeds {temp_threshold}°C threshold. Thermal throttling risk.</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    for title, desc in rca_result["causes"]:
        st.markdown(f"""
        <div style="background:#161b22; border:1px solid #21262d; border-left:4px solid {sev_color}; border-radius:8px; padding:10px 14px; margin-bottom:6px; font-family:'JetBrains Mono',monospace; font-size:0.82rem; color:#e6edf3;">
            <strong>{title}</strong> &nbsp;·&nbsp; <span style="color:#8b949e;">{desc}</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    for i, rec in enumerate(rca_result["recommendations"], 1):
        st.markdown(f"""
        <div style="background:#0d1117; border:1px solid #21262d; border-radius:8px; padding:8px 14px; margin-bottom:6px; font-family:'JetBrains Mono',monospace; font-size:0.8rem; color:#79c0ff;">
            <span style="color:#3fb950;">{i}.</span> {rec}
        </div>
        """, unsafe_allow_html=True)

    log_event("INFO", "RCA", f"Analysis run: severity={sev}, cpu={use_cpu}, mem={use_mem}, disk={use_disk}, temp={cpu_temp}°C")
...
with tab_ai:
    ...
    st.markdown('<p style="color:#8b949e; font-size:0.85rem; font-family:\'JetBrains Mono\',monospace;">Intelligent system diagnostics · Severity analysis · Root cause detection · Solutions</p>', unsafe_allow_html=True)

    if "chat_history" not in st.session_state:
        st.session_state.chat_history = []
        st.session_state.chat_history.append({
            "role": "ai",
            "message": f"""👋 **SentinelOps AI online.**

Current system snapshot:
- 🔲 CPU: **{data['cpu_percent']}%** | 🧠 Memory: **{data['memory_percent']}%** | 💾 Disk: **{data['disk_percent']}%**
- 🌡️ CPU Temp: **{cpu_temp}°C** | ❤️ Health Score: **{health_score}/100**

Ask me anything — *"Analyze CPU"*, *"CPU Temperature"*, *"System Health"*, *"Top processes"*, or type **help** for all commands."""
        })
    ...
    st.markdown('<div style="font-family:\'JetBrains Mono\',monospace; font-size:0.72rem; color:#8b949e; text-transform:uppercase; letter-spacing:0.1em; margin-bottom:8px;">QUICK ANALYSIS</div>', unsafe_allow_html=True)
    ...
    quick_prompts = {
        ...
        "🔍 Root Cause": "Why are there issues?",
        "⚙️ Processes": "Show top processes",
    }
    for col, (label, prompt) in zip([qp1, qp2, qp3, qp4, qp5, qp6], quick_prompts.items()):
        if col.button(label, use_container_width=True):
            st.session_state.chat_history.append({"role": "user", "message": prompt})
            ctx = {**data, "health_score": health_score, "cpu_temp": cpu_temp}
            response = get_ai_response(prompt, ctx)
            st.session_state.chat_history.append({"role": "ai", "message": response})
            log_event("INFO", "AI", f"Query: {prompt[:50]}")
            st.rerun()

    ai_input = st.text_input("💬 Ask the AI...", placeholder="e.g. Analyze CPU · CPU Temperature · Is disk safe? · Help", key="ai_input_box")
    ...
    with send_col:
        if st.button("Send →", use_container_width=True) and ai_input:
            ...
            ctx = {**data, "health_score": health_score, "cpu_temp": cpu_temp}
            response = get_ai_response(ai_input, ctx)
            st.session_state.chat_history.append({"role": "ai", "message": response})
            log_event("INFO", "AI", f"Query: {ai_input[:50]}")
            st.rerun()
    with clear_col:
...
with tab_report:
    ...
    st.markdown('<p style="color:#8b949e; font-size:0.85rem; font-family:\'JetBrains Mono\',monospace;">Generate a professional PDF report with full system health summary, metrics, alerts, and logs.</p>', unsafe_allow_html=True)
    ...
    with r1:
        st.markdown(f"""
        <div style="background:#161b22; border:1px solid #21262d; border-radius:14px; padding:1.4rem 1.6rem; border-left:4px solid #58a6ff;">
            <div style="font-family:'JetBrains Mono',monospace; font-size:0.7rem; color:#8b949e; text-transform:uppercase; letter-spacing:0.1em; margin-bottom:10px;">📋 REPORT CONTENTS</div>
            <div style="font-family:'JetBrains Mono',monospace; font-size:0.82rem; color:#e6edf3; line-height:1.8;">
                ✅ &nbsp;CPU, Memory, Disk metrics<br>
                ✅ &nbsp;CPU Temperature status<br>
                ✅ &nbsp;Health score & status<br>
                ✅ &nbsp;System information<br>
                ✅ &nbsp;Active alerts summary<br>
                ✅ &nbsp;Recent system logs<br>
                ✅ &nbsp;Timestamp & user info<br>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with r2:
        st.markdown(f"""
        <div style="background:#161b22; border:1px solid #21262d; border-radius:14px; padding:1.4rem 1.6rem; border-left:4px solid {hs_col_r};">
            ...
            <div style="font-family:'JetBrains Mono',monospace; font-size:0.85rem; color:#e6edf3; line-height:2;">
                🔲 &nbsp;CPU: <span style="color:{cpu_color}; font-weight:700;">{data['cpu_percent']}%</span><br>
                🧠 &nbsp;Memory: <span style="color:{mem_color}; font-weight:700;">{data['memory_percent']}%</span><br>
                💾 &nbsp;Disk: <span style="color:{disk_color}; font-weight:700;">{data['disk_percent']}%</span><br>
    ...
    if not REPORTLAB_AVAILABLE:
        st.warning("⚠️ `reportlab` library not installed. Run: `pip install reportlab`")
            ...
            with st.spinner("Generating professional PDF report..."):
                try:
                    pdf_bytes = generate_pdf_report(
                        ...
                        health_color_name=hs_label_r,
                        alerts_list=st.session_state.get("alert_history", []),
                        log_entries=st.session_state.get("log_entries", []),
                        cpu_temp=cpu_temp
                    )
                    now_str = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                    filename = f"sentinelops_report_{now_str}.pdf"
                    log_event("INFO", "Report", f"PDF report generated by {st.session_state.current_user}")
                    ...
                    log_event("ERROR", "Report", f"PDF generation failed: {e}")

    st.markdown("<div style='margin:1rem 0;'></div>", unsafe_allow_html=True)
    st.markdown("""
    <div style="background:#0d1117; border:1px solid #21262d; border-radius:10px; padding:0.8rem 1.2rem; font-family:'JetBrains Mono',monospace; font-size:0.78rem; color:#30363d;">
        📌 Reports include a point-in-time snapshot. Generate a fresh report for the latest data. Reports are not stored on the server.
    </div>
    """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════
if is_admin and tab_logs_ref is not None:
    with tab_logs_ref:
        st.markdown("### 🗒️ System Logs")

        log_col1, log_col2, log_col3 = st.columns([2, 1, 1])
        with log_col1:
            log_filter = st.selectbox("Filter by Level", ["ALL", "INFO", "WARNING", "ERROR", "CRITICAL"])
        with log_col2:
            log_component = st.selectbox("Component", ["ALL", "Auth", "CPU", "Memory", "Disk", "CPUTemp", "EmailAlert", "RCA", "AI", "Report", "MultiSys", "Admin", "System", "Metrics"])
        with log_col3:
        ...
        if not entries:
            ...
            log_event("INFO", "Auth", f"Session active: {st.session_state.current_user}")
            log_event("INFO", "Metrics", f"Live metrics: CPU={data['cpu_percent']}%, MEM={data['memory_percent']}%, DISK={data['disk_percent']}%, TEMP={cpu_temp}°C")
            entries = st.session_state.get("log_entries", [])

        filtered = entries
        if log_filter != "ALL":
            filtered = [e for e in filtered if e.get("level") == log_filter]
        if log_component != "ALL":
            filtered = [e for e in filtered if e.get("component") == log_component]
        ...
        for col, val, label, color in [(ls1, total, "Total", "#8b949e"), (ls2, infos, "Info", "#58a6ff"), (ls3, warns, "Warnings", "#e3b341"), (ls4, errors, "Errors", "#f85149")]:
            col.markdown(f"""
            <div style="background:#161b22; border:1px solid #21262d; border-top:2px solid {color}; border-radius:8px; padding:8px 12px; text-align:center;">
                <div style="font-family:'JetBrains Mono',monospace; font-size:1.3rem; font-weight:700; color:{color};">{val}</div>
                <div style="font-family:'JetBrains Mono',monospace; font-size:0.68rem; color:#8b949e; text-transform:uppercase; letter-spacing:0.08em;">{label}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if filtered:
            ...
            for e in filtered[:100]:
                ...
                msg = e.get("message", "")
                log_html += f'<div style="padding:4px 0; border-bottom:1px solid #161b22; color:#8b949e;"><span style="color:#30363d;">{ts}</span> <span style="color:{color};">[{lvl}]</span> <span style="color:#58a6ff;">{comp}</span> → <span style="color:#e6edf3;">{msg}</span></div>'
            log_html += '</div>'
            st.markdown(log_html, unsafe_allow_html=True)
        else:
            st.markdown('<div style="background:#161b22; border:1px solid #21262d; border-radius:10px; padding:1.2rem; text-align:center; color:#8b949e; font-family:\'JetBrains Mono\',monospace; font-size:0.85rem;">No log entries match the current filter.</div>', unsafe_allow_html=True)
        ...
        with dl1:
            if entries:
                txt_lines = [f"{e.get('timestamp','')} [{e.get('level','')}] {e.get('component','')} → {e.get('message','')}" for e in entries]
                txt_data = "\n".join(txt_lines)
                st.download_button("⬇️ Download as .txt", txt_data, "sentinelops_logs.txt", "text/plain", use_container_width=True)
        with dl2:
            if entries:
                export_df = pd.DataFrame(entries)
                csv_data = export_df.to_csv(index=False)
                st.download_button("⬇️ Download as .csv", csv_data, "sentinelops_logs.csv", "text/csv", use_container_width=True)
        with dl3:
            if st.button("🗑️ Clear Logs", use_container_width=True):
                st.session_state.log_entries = []
                log_event("INFO", "Admin", f"Logs cleared by {st.session_state.current_user}")
        ...
        if login_hist:
            ...
            st.info("No login history recorded this session.")
        ...
        if alert_hist:
            ...
            st.info("No alerts triggered this session.")

        if os.path.exists(LOG_FILE):
            st.markdown("---")
            st.markdown("#### 📁 File Log (sentinelops.log)")
            with open(LOG_FILE) as f:
                file_lines = f.readlines()[-50:]
            file_log_txt = "".join(file_lines)
            st.text_area("Last 50 lines from log file", file_log_txt, height=200)

# ══════════════════════════════════════════════════════════
#  TAB 6: MULTI-SYSTEM (ADMIN ONLY)
# ══════════════════════════════════════════════════════════
if is_admin and tab_multisys_ref is not None:
    with tab_multisys_ref:
        st.markdown("### 🌐 Multi-System Monitor")
        st.markdown('<p style="color:#8b949e; font-size:0.85rem; font-family:\'JetBrains Mono\',monospace;">Register and monitor multiple hosts from one dashboard.</p>', unsafe_allow_html=True)
        ...
        for i in range(0, len(systems), cols_per_row):
            ...
            for col, sys in zip(cols, row_systems):
                ...
                if is_local:
                    s_cpu  = data['cpu_percent']
                    s_mem  = data['memory_percent']
                    s_disk = data['disk_percent']
                    s_health = health_score
                    s_color = health_color
                else:
                    s_cpu  = random.uniform(10, 95)
                    s_mem  = random.uniform(20, 90)
                    s_disk = random.uniform(15, 80)
                    s_health = round(100 - (s_cpu*0.4 + s_mem*0.35 + s_disk*0.25), 1)
                    s_color = get_health_color(s_health)
                ...
                col.markdown(f"""
                <div style="background:#161b22; border:1px solid #21262d; border-top:3px solid {s_color}; border-radius:12px; padding:1rem 1.2rem; margin-bottom:0.5rem;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                        <span style="font-family:'JetBrains Mono',monospace; font-weight:700; color:#e6edf3; font-size:0.9rem;">{status_dot} {sys['name']}</span>
                        <span style="font-family:'JetBrains Mono',monospace; font-size:0.7rem; color:#8b949e;">{sys['host']}</span>
                    </div>
                    <div style="margin-bottom:8px;">{tags_html}</div>
                    <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:6px;">
                        <div style="text-align:center;"><div style="font-family:'JetBrains Mono',monospace; font-size:0.68rem; color:#8b949e;">CPU</div><div style="font-family:'JetBrains Mono',monospace; font-size:0.9rem; font-weight:700; color:{get_color(s_cpu)};">{s_cpu:.0f}%</div></div>
                        <div style="text-align:center;"><div style="font-family:'JetBrains Mono',monospace; font-size:0.68rem; color:#8b949e;">MEM</div><div style="font-family:'JetBrains Mono',monospace; font-size:0.9rem; font-weight:700; color:{get_color(s_mem)};">{s_mem:.0f}%</div></div>
                        <div style="text-align:center;"><div style="font-family:'JetBrains Mono',monospace; font-size:0.68rem; color:#8b949e;">DISK</div><div style="font-family:'JetBrains Mono',monospace; font-size:0.9rem; font-weight:700; color:{get_color(s_disk)};">{s_disk:.0f}%</div></div>
                    </div>
                    <div style="margin-top:8px; text-align:center;">
                        <span style="font-family:'JetBrains Mono',monospace; font-size:0.7rem; color:{s_color};">❤️ Health: {s_health}</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        if st.button("➕ Add System", use_container_width=False):
            if new_name and new_host:
                ...
                log_event("INFO", "MultiSys", f"System registered: {new_name} ({new_host})")
                st.success(f"✅ System '{new_name}' registered!")
                st.rerun()
            else:
                st.error("⚠️ Name and Host are required.")
        ...
        if len(systems) > 1:
            ...
            if removable:
                ...
                if st.button("🗑️ Remove"):
                    ...
                    log_event("WARNING", "MultiSys", f"System removed: {to_remove}")
                    st.success(f"Removed '{to_remove}'")
                    st.rerun()

# ══════════════════════════════════════════════════════════
#  TAB 7: ADMIN PANEL
#  — If user reached dashboard via User Portal, they must
#    re-authenticate with admin credentials to access this tab.
# ══════════════════════════════════════════════════════════
if is_admin and tab_admin_ref is not None:
    with tab_admin_ref:
        st.markdown("### 👑 Admin Panel")

        # ── ADMIN RE-AUTH GATE ──
        # Required if admin came via "user dashboard" flow (admin_panel_auth = False)
        if not st.session_state.admin_panel_auth:
            st.markdown("""
            <div style="background:rgba(250,204,21,0.06); border:1px solid rgba(250,204,21,0.3); border-radius:16px; padding:2rem; max-width:480px; margin:2rem auto; text-align:center;">
                <div style="font-size:2rem; margin-bottom:8px;">🔐</div>
                <div style="font-family:'JetBrains Mono',monospace; font-size:1rem; font-weight:700; color:#fbbf24; margin-bottom:6px;">Admin Re-Authentication Required</div>
                <div style="font-family:'JetBrains Mono',monospace; font-size:0.78rem; color:#92400e; margin-bottom:1.5rem;">You must verify admin credentials to access this panel.</div>
            </div>
            """, unsafe_allow_html=True)

            re_col1, re_col2, re_col3 = st.columns([1.5, 1, 1.5])
            with re_col2:
                re_uname = st.text_input("👑 Admin Username", key="re_admin_uname", placeholder="admin")
                re_pass  = st.text_input("🔐 Admin Password", type="password", key="re_admin_pass", placeholder="••••••••")
                if st.button("🔓 Verify & Enter Admin Panel", use_container_width=True):
                    users = admin_config.get("users", {})
                    ph = hashlib.sha256(re_pass.encode()).hexdigest()
                    if re_uname in users and users[re_uname]["password_hash"] == ph and users[re_uname]["role"] == "admin":
                        st.session_state.admin_panel_auth = True
                        log_event("INFO", "Auth", f"Admin panel re-auth successful for '{re_uname}'")
                        record_login(re_uname, "admin", True)
                        st.success("✅ Admin access granted!")
                        st.rerun()
                    else:
                        log_event("WARNING", "Auth", f"Admin panel re-auth failed for '{re_uname}'")
                        st.error("❌ Invalid admin credentials.")
            st.stop()

        # ── ADMIN PANEL CONTENT (after re-auth) ──
        admin_tabs = st.tabs([
            "⚙️ Thresholds",
            "📧 Email Alerts",
            "🔊 Voice Alerts",
        ...
        with admin_tabs[0]:
            ...
            st.markdown('<p style="color:#8b949e; font-size:0.82rem; font-family:\'JetBrains Mono\',monospace;">Set the values that trigger alerts.</p>', unsafe_allow_html=True)
        ...
        with admin_tabs[1]:
            ...
            sender_email  = st.text_input("Sender Email", value=ec.get("sender", ""), placeholder="alerts@yourdomain.com")
            ...
            with col_save:
                if st.button("💾 Save Email Config", use_container_width=True):
                    ...
                    save_admin_config(admin_config)
                    log_event("INFO", "Admin", "Email config updated")
                    st.success("✅ Email configuration saved!")
            with col_test:
                if st.button("📤 Send Test Email", use_container_width=True):
                    cfg = admin_config.get("email_config", {})
                    ok, msg = send_email_alert(
                        "Test Alert",
                        "<h2>SentinelOps Test</h2><p>Email alerts are working correctly.</p>",
                        cfg
                    )
        ...
        with admin_tabs[2]:
            ...
            voice_enabled = st.checkbox("Enable Voice Alerts (Browser TTS)", value=admin_config.get("voice_alerts", True))
            if st.button("💾 Save Voice Settings"):
                admin_config["voice_alerts"] = voice_enabled
                save_admin_config(admin_config)
                log_event("INFO", "Admin", f"Voice alerts: {'enabled' if voice_enabled else 'disabled'}")
                st.success("✅ Saved!")
            if voice_enabled:
                st.markdown("""
                <div style="background:#1a2d1e; border:1px solid #3fb950; border-radius:10px; padding:0.8rem 1.2rem; font-family:'JetBrains Mono',monospace; font-size:0.8rem; color:#3fb950; margin-top:1rem;">
                    ✅ Voice alerts are <strong>ENABLED</strong>. Critical threshold breaches will be spoken via browser TTS.
                </div>
                """, unsafe_allow_html=True)
                ...
                with vc2:
                    if st.button("🌡️ Test Temp Alert"):
                        inject_voice_alert("Warning! Your CPU temperature is high. Please check your cooling system.", "CRITICAL")
                        st.info("▶️ Playing CPU temperature alert...")
            else:
                st.markdown("""
                <div style="background:#2d1a1a; border:1px solid #f85149; border-radius:10px; padding:0.8rem 1.2rem; font-family:'JetBrains Mono',monospace; font-size:0.8rem; color:#f85149; margin-top:1rem;">
                    🔇 Voice alerts are <strong>DISABLED</strong>.
                </div>
                """, unsafe_allow_html=True)
        ...
        with admin_tabs[3]:
            ...
            users = admin_config.get("users", {})
            user_data = [{"username": u, "role": v["role"]} for u, v in users.items()]
            ...
            if st.button("💾 Save User", use_container_width=True):
                if new_uname and new_upass:
                    admin_config["users"][new_uname] = {
                        "password_hash": hashlib.sha256(new_upass.encode()).hexdigest(),
                        "role": new_urole
                    }
                    save_admin_config(admin_config)
                    log_event("INFO", "Admin", f"User '{new_uname}' saved with role '{new_urole}'")
        ...
        with admin_tabs[4]:
            ...
            if st.button("💾 Save Config", use_container_width=True):
                ...
                save_admin_config(admin_config)
                log_event("INFO", "Admin", f"System config updated: refresh={refresh_int}s, retention={retention}d")
            ...
            with dz1:
                if st.button("🗑️ Clear All In-Memory Logs"):
                    st.session_state.log_entries = []
                    log_event("INFO", "Admin", "Logs cleared from Danger Zone")
                    st.success("✅ Logs cleared.")
            with dz2:
        ...
        with admin_tabs[5]:
            ...
            def overview_card(col, title, value, sub, color, icon):
                col.markdown(f"""
                <div style="background:#161b22; border:1px solid #21262d; border-top:3px solid {color}; border-radius:12px; padding:1.2rem 1.4rem; text-align:center;">
                    <div style="font-size:1.6rem;">{icon}</div>
                    <div style="font-family:'JetBrains Mono',monospace; font-size:0.68rem; color:#8b949e; text-transform:uppercase; letter-spacing:0.1em; margin:4px 0;">{title}</div>
                    <div style="font-family:'JetBrains Mono',monospace; font-size:1.6rem; font-weight:700; color:{color};">{value}</div>
                    <div style="font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:#8b949e; margin-top:4px;">{sub}</div>
                </div>
                """, unsafe_allow_html=True)

            hs_label_ov = "HEALTHY" if health_score >= 70 else ("WARNING" if health_score >= 50 else "CRITICAL")
            overview_card(ov1, "CPU Status",    f"{data['cpu_percent']}%",  f"Threshold: {admin_config['alert_thresholds'].get('cpu',80)}%",      cpu_color,    "🔲")
            overview_card(ov2, "Memory Status", f"{data['memory_percent']}%",f"Threshold: {admin_config['alert_thresholds'].get('memory',85)}%",    mem_color,    "🧠")
            overview_card(ov3, "CPU Temp",      f"{cpu_temp}°C",             f"Threshold: {admin_config['alert_thresholds'].get('cpu_temp',80)}°C", temp_color,   "🌡️")
            overview_card(ov4, "Health Score",  f"{health_score}/100",       hs_label_ov,                                                           health_color, "❤️")
            ...
            cfg_display = {
                ...
                "Voice Alerts":       "Enabled" if admin_config.get("voice_alerts") else "Disabled",
                "Email Alerts":       "Enabled" if admin_config.get("email_config", {}).get("enabled") else "Disabled",
            ...
            cfg_df = pd.DataFrame(list(cfg_display.items()), columns=["Setting", "Value"])
            st.dataframe(cfg_df, use_container_width=True, hide_index=True)

        # ── Alert Monitor ──
        with admin_tabs[6]:
            st.markdown("#### 🚨 Alert Monitor")
            st.markdown('<p style="color:#8b949e; font-size:0.82rem; font-family:\'JetBrains Mono\',monospace;">Real-time view of all triggered alerts this session.</p>', unsafe_allow_html=True)

            alert_hist = st.session_state.get("alert_history", [])
            if not alert_hist:
                st.markdown("""
                <div style="background:#161b22; border:1px solid #21262d; border-radius:10px; padding:1.5rem; text-align:center; color:#3fb950; font-family:'JetBrains Mono',monospace;">
                    ✅ No alerts fired this session — system is healthy!
                </div>
                """, unsafe_allow_html=True)
            else:
                # Summary counts
                ...
                crit_count = sum(1 for a in alert_hist if a.get("severity") == "CRITICAL")
                warn_count = sum(1 for a in alert_hist if a.get("severity") == "WARNING")
                am1.metric("Total Alerts", total_alerts)
                am2.metric("Critical", crit_count)
                am3.metric("Warnings", warn_count)
                ...
                for a in alert_hist[:20]:
                    ...
                    st.markdown(f"""
                    <div style="background:#161b22; border:1px solid #21262d; border-left:4px solid {a_color}; border-radius:8px; padding:10px 14px; margin-bottom:6px; font-family:'JetBrains Mono',monospace; font-size:0.8rem; display:flex; justify-content:space-between; align-items:center;">
                        <span>{a_icon} <span style="color:{a_color}; font-weight:700;">[{sev}]</span> <span style="color:#58a6ff;">{a.get('component','')}</span> → <span style="color:#e6edf3;">{a.get('message','')}</span></span>
                        <span style="color:#30363d; font-size:0.72rem;">{ts}</span>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            if st.button("🗑️ Clear Alert History", use_container_width=False):
                st.session_state.alert_history = []
                log_event("INFO", "Admin", f"Alert history cleared by {st.session_state.current_user}")
        ...
        with admin_tabs[7]:
            ...
            st.markdown('<p style="color:#8b949e; font-size:0.82rem; font-family:\'JetBrains Mono\',monospace;">Trigger test alerts, reset logs, and simulate system events.</p>', unsafe_allow_html=True)

            st.markdown("##### 🔔 Trigger Test Alerts")
            cp1, cp2, cp3 = st.columns(3)

            with cp1:
                if st.button("⚡ Trigger CPU Warning", use_container_width=True):
                    record_alert("WARNING", "CPU", f"TEST: CPU usage at 82% — threshold {thresholds['cpu']}%")
                    log_event("WARNING", "CPU", "TEST ALERT: CPU warning triggered by admin")
                    st.warning("⚡ Test CPU Warning alert injected.")
                    if admin_config.get("voice_alerts"):
                        inject_voice_alert("Test alert: CPU usage is high.", "WARNING")

            with cp2:
                if st.button("🔥 Trigger CPU Critical", use_container_width=True):
                    record_alert("CRITICAL", "CPU", f"TEST: CPU usage at 95% — CRITICAL threshold exceeded")
                    log_event("CRITICAL", "CPU", "TEST ALERT: CPU critical triggered by admin")
                    st.error("🔥 Test CPU Critical alert injected.")
                    if admin_config.get("voice_alerts"):
                        inject_voice_alert("Critical alert! CPU usage is critically high!", "CRITICAL")

            with cp3:
                if st.button("🌡️ Trigger Temp Alert", use_container_width=True):
                    record_alert("CRITICAL", "CPUTemp", f"TEST: CPU Temperature at 92°C — threshold {temp_threshold}°C")
                    log_event("CRITICAL", "CPUTemp", "TEST ALERT: CPU temperature critical triggered by admin")
                    st.error("🌡️ Test CPU Temperature alert injected.")
                    if admin_config.get("voice_alerts"):
                        inject_voice_alert("Warning! Your CPU temperature is high. Please check your cooling system.", "CRITICAL")
            ...
            with cp4:
                if st.button("🧠 Trigger Memory Warning", use_container_width=True):
                    record_alert("WARNING", "Memory", f"TEST: Memory at 88% — threshold {thresholds['memory']}%")
                    log_event("WARNING", "Memory", "TEST ALERT: Memory warning triggered by admin")
                    st.warning("🧠 Test Memory Warning alert injected.")
                    if admin_config.get("voice_alerts"):
                        inject_voice_alert("Test alert: Memory usage is high.", "WARNING")

            with cp5:
                if st.button("💾 Trigger Disk Warning", use_container_width=True):
                    record_alert("WARNING", "Disk", f"TEST: Disk at 93% — threshold {thresholds['disk']}%")
                    log_event("WARNING", "Disk", "TEST ALERT: Disk warning triggered by admin")
                    st.warning("💾 Test Disk Warning alert injected.")
                    if admin_config.get("voice_alerts"):
                        inject_voice_alert("Test alert: Disk usage is high.", "WARNING")

            with cp6:
                if st.button("📢 Test All Alerts", use_container_width=True):
                    for sev, comp, msg in [
                        ("WARNING", "CPU", "TEST: CPU at 82%"),
                        ("WARNING", "Memory", "TEST: Memory at 88%"),
                        ("CRITICAL", "CPUTemp", "TEST: CPU Temp at 92°C"),
                    ]:
                        record_alert(sev, comp, msg)
                        log_event(sev, comp, f"TEST ALERT: {msg}")
                    st.error("📢 All test alerts injected! Check Alert Monitor tab.")
                    if admin_config.get("voice_alerts"):
                        inject_voice_alert("System diagnostic complete. Multiple test alerts triggered.", "WARNING")
            ...
            with rc1:
                if st.button("🗑️ Reset Logs", use_container_width=True):
                    st.session_state.log_entries = []
                    log_event("INFO", "Admin", f"All logs reset by {st.session_state.current_user}")
                    st.success("✅ Logs reset.")

            with rc2:
                if st.button("🗑️ Reset Alerts", use_container_width=True):
                    st.session_state.alert_history = []
                    log_event("INFO", "Admin", f"Alert history reset by {st.session_state.current_user}")
                    st.success("✅ Alert history reset.")

            with rc3:
                if st.button("🔐 Lock Admin Panel", use_container_width=True):
                    st.session_state.admin_panel_auth = False
                    log_event("INFO", "Auth", f"Admin panel locked by {st.session_state.current_user}")
                    st.success("🔒 Admin panel locked. Re-authentication required.")
                    st.rerun()

            st.markdown("---")
            st.markdown("##### ℹ️ System Info")
            st.markdown(f"""
            <div style="background:#0d1117; border:1px solid #21262d; border-radius:10px; padding:1rem 1.4rem; font-family:'JetBrains Mono',monospace; font-size:0.8rem; color:#8b949e; line-height:2;">
                🖥️ &nbsp;Hostname: <span style="color:#e6edf3;">{platform.node()}</span><br>
                🐍 &nbsp;Python: <span style="color:#e6edf3;">{platform.python_version()}</span><br>
                🖥️ &nbsp;OS: <span style="color:#e6edf3;">{platform.system()} {platform.release()}</span><br>
                ⚙️ &nbsp;CPU Cores (Physical): <span style="color:#e6edf3;">{psutil.cpu_count(logical=False)}</span><br>
                ⚙️ &nbsp;CPU Cores (Logical): <span style="color:#e6edf3;">{psutil.cpu_count(logical=True)}</span><br>
                🧠 &nbsp;Total RAM: <span style="color:#e6edf3;">{round(psutil.virtual_memory().total/(1024**3), 2)} GB</span><br>
                💾 &nbsp;Total Disk: <span style="color:#e6edf3;">{round(psutil.disk_usage('/').total/(1024**3), 2)} GB</span><br>
                👤 &nbsp;Admin: <span style="color:#fbbf24;">{st.session_state.current_user}</span><br>
            </div>
            """, unsafe_allow_html=True)
...
st.markdown(f"""
<div style="position:fixed; bottom:12px; right:16px; font-family:'JetBrains Mono',monospace; font-size:0.68rem; color:#21262d; z-index:999;">
    SentinelOps v2.0 · {st.session_state.current_user} [{st.session_state.user_role}] · Auto-refresh: {refresh_interval}s
</div>
""", unsafe_allow_html=True)